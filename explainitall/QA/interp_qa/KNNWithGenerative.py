...
class PromptBot():
  ...
  """
  def __init__(self, knn, sbert, fred, texts, max_words = 50, mean = None, std = None, vect_transformer = None, dim = None, n_neighbors = 3, eps = 1e-200, device = 'cuda'):
  ...
  def __generate__(self, doc, q):
    ...
    output_ids = self.fred.get_model().generate(
        **t,  do_sample=True, temperature=0.2, max_new_tokens=256, top_p=0.95, top_k=15, repetition_penalty=1.03, no_repeat_ngram_size=3
    )[0]
