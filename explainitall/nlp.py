...
class WordProcessor:
    """
    # self.wp.get_normal_form_or_none() # word_cleaner(self, word, no_prefix=True)
    # self.wp.get_embeddable_word_or_none()  # word_cleaner(self, word, no_prefix=False)
    ...
    def get_normal_form_or_none(self, word: str):
        ...
        if not clean_word:
            return None
        word_morph = self.get_morph_or_none(clean_word)  # Fixed: Use clean_word instead of word
        if not word_morph:
            return None
    ...
    def get_embeddable_word_or_none(self, word: str):  # Fixed: Corrected the method name
